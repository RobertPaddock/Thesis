...
%The last few MHD equations are obtained by considering Maxwell's equations. Ampere's law, $\frac{\partial \vec{E}}{\partial t} = c^2 (\div (\grad \times \vec{B} - \mu_0 \vec{J}) )$, is reduced by the the MHD assumptions (particularly that of long time scales) to give )
%\begin{equation} \frac{\partial \vec{E}}{\partial t} = c^2 (\div (\grad \times \vec{B} - \mu_0 \vec{J}) ) \end{equation}
