...
Consider the evolution of the plasma distribution function $f$ with time. First, consider the collisionless case. Taking the derivative $\frac{df}{dt}$ and rewriting the variables, the collisionless Boltzmann equation is obtained,
...
The Boltzmann equation describes the evolution of the distribution function, which is the complete mathematical description of the plasma. Fluid variables, which vary as a function of $\vec{r}$ and $t$ only, can be obtained by taking moments of velocity of the distribution function (i.e. performing the integral $ \int^{\infty}_{-\infty} v^i f d\vec{v} $, where $i$ represents the moment being taken). The first three moments return the particle density $n$, fluid velocity $\vec{u}$, and pressure tensor $\vec{P}$:
...
Fluid equations governing the evolution of each of these variables can also be derived\footnote{This discussion focussed on deriving them from the kinetic description of plasmas, but they can be derived from first principles by considering the fluid evolution instead.}, by taking the equivalent moment of the Boltzmann equation. Under the fluid approximation, these equations are solved to describe the evolution of the plasma as a fluid; no knowledge of the distribution function is required.
...
This section will outline briefly how the ideal MHD equations (the equations that govern magnetohydrodynamics) can be obtained. A simple form will be derived based on only the zeroth and first order moment equations (continuity and equation of motion), but alternative formulations can be derived which also include the energy equation (doing so requires a heat flux equation in order to close the equations). Continuity equations for the single-fluid can be obtained by summing the individual continuity equations for the ions and electrons. This gives the \textit{conservation of mass} equation,
...
Such equations can be solved as a compete set in a simulation code to describe the fluid behaviour of a plasma. More accurate variations of this formalism can include additional terms in the equations (accounting for collisions and resistivity), or include the energy equation as well (which requires a further heat transport equation for closure). 

As the full plasma distribution function does not need to be evaluated, fluid codes (which utilise the fluid approximation) can be simulated over the time and length scales required for a full ICF implosion, making them an essential tool for investigating these reactions. However, it is also important to note their limitations. In particular, the assumptions that the time and length scales of interest are long means that fast phenomena (on the timescale of the phenomena) cannot be simulated - including the plasma oscillations discussed in \hl{SECTION}. There are other codes that can simulate these effects, but they cannot be used to simulate the necessary timescales - an example would be Vlasov codes, which solve the Vlasov equation directly and return the full distribution function. For this reason, effective plasma physics simulations require careful planning to ensure the necessary physics is captured for the scenario being simulated.
...
The previous section outlined how the fluid approximation can be used to describe the evolution of plasmas over large time and length scales. However, it also demonstrated how the fluid equations themselves are not complete, and require further models (the equation of state, or the heat transport equation) in order to be solvable. In reality, simulation codes based on this approach typically involve further models, which are used to improve accuracy and are included by adding additional terms to the MHD equations. 

This section will give provide a brief overview of some of the key physics that is important in describing plasmas (such as laser absorbtion and ionisation) and how these are included in fluid simulations, as well as outlining some of the additional physics models that are added (such as heat and radiation transport, and the equation of state). As this thesis will  present a large number of hydrodynamic simulations using the 1D radiation-hydrodynamics code HYADES, the specific models that are used in this code will be indicated. This section is intended to provide a brief overview of this physics, and a more complete description can be found in  \cite{Colvin2013} (on which this section is based).
